...
def connect_and_scrape():
    try:
        ...
        options.add_argument("user-agent=Mozilla/5.0 ... Chrome/115 Safari/537.36")
        ...
        if len(tables) >= 3:
            print("✅ Tables loaded. Extracting data...")
            ...
            print(f"📆 Data Date: {scraped_date}")

            # Save to DB
            save_to_postgres(df_nse, "nse")
            save_to_postgres(df_combined, "combined_nse_bse_msei")
        else:
            print(f"⚠️ Expected at least 3 tables, found {len(tables)}")
