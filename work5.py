...
#             save_to_excel(df_nse, f"{excel_folder}/nse_data_{today}.xlsx")
#             save_to_excel(df_combined, f"{excel_folder}/combined_data_{today}.xlsx")
...
#         # ✅ After scraping, verify data from PostgreSQL and export to Excel
#     try:
#         conn = psycopg2.connect(
...
#         print("\n📊 NSE Table Preview:")
#         print(df_nse.head())
#         df_nse.to_excel(f"{excel_folder}/nse_data_from_db_{today}.xlsx", index=False)

#         # 🔍 Read from combined table
#         df_combined = pd.read_sql("SELECT * FROM combined_nse_bse_msei", conn)
#         print("\n📊 Combined NSE/BSE/MSEI Table Preview:")
#         print(df_combined.head())
#         df_combined.to_excel(f"{excel_folder}/combined_data_from_db_{today}.xlsx", index=False)
...
#             # Save to Excel with actual scraped date
#             save_to_excel(df_nse, f"{excel_folder}/nse_data_{scraped_date}.xlsx")
#             save_to_excel(df_combined, f"{excel_folder}/combined_data_{scraped_date}.xlsx")
...
#         # 🔍 Read and print NSE table
#         df_nse = pd.read_sql("SELECT * FROM nse", conn)
#         if not df_nse.empty:
#             latest_date_nse = df_nse["DATE"].iloc[0].strip().replace("/", "-").replace(" ", "_")
#             print(f"\n📊 NSE Table Preview (Date: {latest_date_nse}):")
#             print(df_nse.head())
#             df_nse.to_excel(f"{excel_folder}/nse_data_from_db_{latest_date_nse}.xlsx", index=False)

#         # 🔍 Read and print Combined table
#         df_combined = pd.read_sql("SELECT * FROM combined_nse_bse_msei", conn)
#         if not df_combined.empty:
#             latest_date_combined = df_combined["DATE"].iloc[0].strip().replace("/", "-").replace(" ", "_")
#             print(f"\n📊 Combined Table Preview (Date: {latest_date_combined}):")
#             print(df_combined.head())
#             df_combined.to_excel(f"{excel_folder}/combined_data_from_db_{latest_date_combined}.xlsx", index=False)
